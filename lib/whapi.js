...
export function isMonitoredGroup(groupName) {
  ...
  const monitoredGroups = process.env.MONITORED_GROUPS?.split(',').map(g => g.trim().toLowerCase()) || [];

  // MONITORED_GROUPSが空の場合は全グループを監視
  if (monitoredGroups.length === 0) return true;

  return monitoredGroups.some(name => groupName.toLowerCase().includes(name));
}
